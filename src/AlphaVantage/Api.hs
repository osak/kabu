...
instance FromJSON StockHistory where
  parseJSON = withObject "StockHistory" $ \v -> StockHistory
    <$> v .: "Meta Data"
    <*> (fetchSeries v >>= parseSeries)
    where
      ...
      parsePrice day = withObject "Price" $ \v -> Price
        (fromJust (parseTimeM True defaultTimeLocale "%Y-%m-%d" (unpack day) :: Maybe Day))
        <$> fmap read (v .: "1. open")
        <*> fmap read (v .: "2. high")
        <*> fmap read (v .: "3. low")
        <*> fmap read (v .: "4. close")
